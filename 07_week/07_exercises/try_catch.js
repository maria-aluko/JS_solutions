...
function task3() {
  try {
    throw new Error('Random error text.');
  } catch (error) {
...
function task4() {
  try {
    ...
    try {
      throw new Error('This is the error inside nested try block');
    } catch (innerError) {
      console.error('Caught inner try:', innerError.message);
    }
    throw new Error('Error in outer try block');
  } catch (outerError) {
    console.error('This caught the outer try:', outerError.message);
  } finally {
...
function task6(obj, key) {
  try {
    console.log(obj[key]); // why like that tho?
  } catch (error) {
...
task6({ firstName: 'Maria' }, 'firstName');
...
task8(-1); // Caught an error: Number must be positive. Task completed
task8(2); // console logs 2 and then task completed
...
function task9(number) {
  try {
    let numbers = new Array(number); // this needs to be inside the try-catch thing
    if (number < 0) {
      throw new RangeError('Array length must be non-negative');
    }
    console.log(numbers); // just here to see what it displays
  } catch (error) {
